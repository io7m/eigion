...
package com.io7m.eigion.server.service.domaincheck;
...
public final class EISDomainChecking
  implements EISDomainCheckingType
{
    ...
    final EIGroupCreationRequest result)
  {
    try (var connection = this.database.openConnection(EIGION)) {
      try (var transaction = connection.openTransaction()) {
        ...
        if (result.status() instanceof EIGroupCreationRequestStatusType.Succeeded) {
