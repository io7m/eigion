...
public final class EIATransactionServlet extends EIAAuthenticatedServlet
{
  private static final Logger LOG =
    LoggerFactory.getLogger(EIATransactionServlet.class);
    ...
    throws Exception
  {
    ...
    try (var input = this.limits.boundedMaximumInput(request, 10_000_000)) {
      final var data = input.readAllBytes();
      final var message = this.messages.parse(data);
      if (message instanceof EISA1Transaction command) {
    ...
    EIPasswordException
  {
    ...
    final var subCommands =
      command.commands();
    final var responses =
      new ArrayList<EISA1ResponseType>(subCommands.size());

    var failed = false;
    int httpStatus = 200;
    for (final var subCommand : subCommands) {
      final var result =
        this.executor.execute(context, subCommand);
    ...
    this.sends()
      .send(
        servletResponse,
        httpStatus,
        new EISA1TransactionResponse(requestId, responses)
      );
  }
}
