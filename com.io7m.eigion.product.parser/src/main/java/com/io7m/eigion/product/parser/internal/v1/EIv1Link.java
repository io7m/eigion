...
@JsonSerialize
@JsonDeserialize
public final class EIv1Link
  implements EIv1FromV1Type<EILink>
{
  ...
  @JsonCreator
  public EIv1Link(
    final @JsonProperty(value = "Relation", required = true) String inRelation,
    final @JsonProperty(value = "Location", required = true) URI inLocation)
  {
    this.relation =
      Objects.requireNonNull(inRelation, "relation");
    this.location =
      Objects.requireNonNull(inLocation, "location");
  }

  @Override
  public Optional<EILink> toProduct(
    final URI source,
    final Consumer<ParseStatus> errorConsumer)
  {
    try {
      return Optional.of(new EILink(this.relation, this.location));
    } catch (final Exception e) {
      errorConsumer.accept(
        ParseStatus.builder()
          .setSeverity(ParseSeverity.PARSE_ERROR)
          .setErrorCode("invalid-link")
