...
public final class EIArbGroupCreationRequestStatusProvider
  extends EIArbAbstractProvider
{
  ...
  public EIArbGroupCreationRequestStatusProvider()
  {
  ...
  {
    return targetType.isOfType(EIGroupCreationRequestStatusType.class);
  }
    ...
    final SubtypeProvider subtypeProvider)
  {
    ...
    return Set.of(Combinators.combine(t, t, d, s).as((ts, tc, succ, text) -> {
      if (succ.doubleValue() > 0.5) {
        return new Succeeded(ts, tc);
      }
      return new Failed(ts, tc, text);
    }));
  }
}
